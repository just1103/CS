...
Last Edited Time: July 16, 2021 9:50 PM
Property: 경기대_이은정T
Type: CS
    ...
    - 2. 해결방법-1. Student 객체 생성자 지정
        ...
        void readAll() {		
        		...
        		while(true) {
        			...
        			st = new Student(id); // 생성자로 id를 설정 -> 1) Student Class 내부에 동일한 이름의 메소드를 생성해야 한다. (생성자 정의) 또한 2) read 함수에서 id 읽기를 삭제한다.
        			st.read(scan); 
        			studentList.add(st);
        		}
...
- Class 설계원칙-3. SRP (Singble Resposibility Principle, 단일책임원칙)
    - Class는 한 가지만 책임진다.
...
- 키워드 검색 - 키워드를 입력하면, 학생 객체 중 해당 키워드에 매치되는 필드가 있으면 출력하는 기능
    - 단계별 구현
        - 1. 이름 검색
            ...
            - 이름에 입력한 string과 각 학생의 이름을 비교하여, 해당되는 학생이 있으면 학생을 출력하고, 없으면 "없다"고 출력한다.
            ...
            - OOP이면 학생의 필드 (이름 정보)는 Student Class에서 처리 (비교/검사/수정)해야 한다. 이렇게 하면 Department Class를 최소 기능으로 유지 가능하다.
        ...
        - 2. 이름 또는 학번 검색
            ...
            boolean matches(String kwd) {
            		if(name.equals(kwd)) // 이름 match 확인 
            			return true;
            		if(kwd.equals(id + "")) // 학번 match 확인. *어떻게 1개의 parameter로 2개 필드를 비교하지? -> this.id를 int->String으로 형변환 (+"" 자동변환 활용)하여 처리한다!
            ...
            boolean matches(String kwd) {
            		if(name.equals(kwd)) // 이름 match 확인 
            			return true;
            		if(kwd.equals(id + "")) // 학번 match 확인. *어떻게 1개의 parameter로 2개 필드를 비교하지? -> this.id을 int->String으로 형변환 (+"" 자동변환 활용)하여 처리한다!
            			return true;	
        ...
        - 3. 모든 필드 검색 (통합검색 기능) 🍎🍎🍎
            ...
            - [ ]  검색키워드 여러 개 빈칸으로 구분 : 0234 2 <- 입력 - 핸드폰 번호나 이름 full로 입력해야 출력되는데??

...
- 학생의 필드인 name을 어떻게 접근? - 학생에서 name을 돌려주는 get 함수를 만드는 방법도 있으나, st.name으로 바로 접근하는 것도 가능하다.
...
- 학생에 팀 필드 추가
    ...
    void setTeam(Team t) { // 입력 이후 나중에 팀을 지정하는 구조이므로 setter가 필요하다. (setter 사용은 최소화하는 게 좋지만)
    	// ... myTeam = t; (?)
    }
    ```
...
- 과목 및 수강정보 처리 기능
- 과목 정보 입력 → 과목 정보에 대한 Class 추가
- 학생의 수강정보 입력 → 학생에 수강정보 필드 생성
- 검색 기능 추가
- 과목을 수강하는 학생 검색
- 학생이 수강하는 과목 검색
- 주어진 요일에 수업이 있는 학생 검색

- 기능 설계
    - 과목 Class 추가
    - 데이터로부터 Class 정의
    - 필드 생성 (과목코드, 과목명, 학년, 요일, 시간)
    - 입력 read 메소드와 출력 print 메소드 생성

        ```java
        // 과목 Data - 과목코드, 과목명, 학년, 요일, 시간
        DD754 창의기초설계 1 월 123
        DD123 C프로그래밍 1 화 678
        DD890 시스템소프트웨어 2 수 123
        AS134 컴퓨터네트워크 2 화 123
        DD982 자료구조설계 2 목 678
        ```

    - 학과 Class 역할
        - 학생 정보 및 과목 정보를 관리
        - 학생을 읽어 List에 저장, 입출력 (readAllStudents(), printAllStudents())
        - 과목을 읽어 List에 저장, 입출력 (readAllLectures(), printAllLectures())
    - 학생 Class 역할
        - 학생의 수강정보를 저장
        - 학생 데이터에서 수강하는 과목코드를 제공
        - 학생이 과목 여러 개를 가지는 구조

        ```java
        // 학생 Data
        201611012 김성관 컴퓨터공학과 2 DD754 DD123 DD890 0 // 과목코드가 끝날 때 '0' 입력
        201711311 김재훈 컴퓨터공학과 3 DD754 AS134 DD982 0
        0
        ```

        ```java
        // 기존 학생정보 외에 '수강정보를 담는 arrayList'를 생성한다. 
        // 기존 학생정보 : (id, name, phone, year, score)
        // 신규 수강정보 : (과목코드, 과목명, 학년, 요일, 시간)
        ...
        ArrayList<Lecture> registeredList = new ArrayList<>() // 과목정보를 담는 registeredList 필드 추가 - Lecture Class의 과목이 먼저 입력되어 있어야 한다.
        ...
        void read(Scanner scan, int id, Department department) {
        		...
        		while (true) {
        				code = scan.next(); // 학생이 신청한 과목의 코드를 입력받음
        				
        				if (code.equals("0"))
        					break;
        				lecture = department.findLecture(code); // Department Class의 findLecture 메소드를 호출한다.
...
- UML 다이어그램
    ...
    ![OOP%20(Java)%20dd6061d9d8954db2839d6acfe1958a90/Untitled%2013.png](OOP%20(Java)%20dd6061d9d8954db2839d6acfe1958a90/Untitled%2013.png)

    - 왼쪽 그림 : Sequence 다이어그램 - 흐름도, 객체 간 메세지 전달을 시간적 흐름에서 분석
    *readAllStudents 내용
    - Student가 Department에게 findLecture를 요구한다. `lecture = department.findLecture(code);`
       → Student의 read 메소드는 그 결과를 받아서 registeredList에 추가한다. `registeredList.add(lecture);`
    - Department는 findLecture를 확인하기 위해 Lecture에게 matches 확인을 요청한다. (그 이후 findLecture가 마무리됨)
       → 최종적으로 Department는 studentList에 추가한다.
    - 오른 그림 : Class 다이어그램 (시스템의 구조)

- 참고 - 다이어그램
    - Class 다이어그램
    - Class 필드 및 메소드
    - Class 간의 관계 (해당 Class 객체의 참조를 필드로 가지는 경우 → 화살표로 표시한다. ex. studentList, registeredList)
    - 프로그램 구조를 한 눈에 파악
    - Sequence 다이어그램
    - Class 메소드 간의 호출관계
    - 호출관계가 복잡할 때 유용함
...
- 참고 - OOP의 기능 확장
    - 새로운 기능을 확장 설계할 때 고려할 요소
    - 추가되는 데이터
    - Class 추가 여부
    - 어떤 Class에 어떤 필드를 추가
    - Class 간의 관계 (참조 필드)는 어떤 구조
    - 순서
    - 간단하고 독립적인 Class부터 먼저 구현
    - 객체 접근방법 : 누가 그 객체를 생성하고 관리하는가
    - 객체 간 관계 : 참조를 어떻게 찾는가

### 학생의 수강과목 출력

- 구현 : Student Class의 기존 print 메소드를 확장
- 출력 형태

    ```java
    201611012 김성관 010-2342-1234 (2학년) <- 기존 학생정보를 출력하면서
    		DD754 창의기초설계 1 월 123 <- Lecture를 통해서 강의정보를 출력시킨다. (Tap 출력)
    		DD123 C프로그래밍 1 화 678
    		DD834 시스템소프트웨어 2 수 123
    		DD982 자료구조설계 2 목 678

    201711311 김재훈 010-1234-5678 (3학년)
    		DD754 창의기초설계 1 월 123
    		AS134 컴퓨터네트워크 2 화 123
    		DD982 자료구조설계 2 목 678
    ```
    ...
    // Student Class
    void print() {
    		System.out.printf("학번 %d, 이름 %s, 연락처 %s (%d학년) ", id, name, phone, year); // 기존 학생정보 출력
    		if (score > 0) // 점수가 있으면 출력하고, 없으면 출력하지 않음 
    				System.out.printf("점수 %d점\n", score);
    		System.out.println(); // 모두 출력 이후 줄바꿈 1번 
    		
    		for (Lecture lecture : registeredList) {
    				System.out.print("\t"); // Tap 출력
    				lecture.print()  // Lecture Class의 print 메서드를 호출한다.
    		}
    }
    ```
...
- 객체 간 연관관계 검색
    - 학생이 수강하는 과목 정보
    - 학생은 과목을 가지고 있으므로 바로 접근이 가능하다.
    - 과목의 수강학생 출력
    - 과목은 학생을 가지고 있지 않다. 더 복잡한 과정이 필요하다.

        ```java
        >> 검색할 과목 : 자료구조설계 <- 입력
        [DD982] 자료구조설계 2학년 금요일/678 <- 출력
        		201611012 김성관 010-2342-1234 (2학년)
        		201711311 김재훈 010-1234-5678 (3학년)
        ```
...
- 사용자가 검색할 과목명을 입력한다.
- 과목명으로 과목 객체를 찾는다.
- 학생들에게 한 명씩 해당 과목을 수강하는지 물어본다.
- 수강하는 경우 해당 학생의 정보를 출력한다.
    - 과목객체 : 과목 찾기
    - 과목명으로 비교하여 일치 여부를 돌려준다. (Lecture.matchesLectures)
    - 학생객체 : 그 과목을 수강하는지 알려줌
    - 학생 객체는 과목 객체를 받아서 해당 과목을 자기가 수강하는지, 즉 registeredList에 있는지 여부를 돌려준다. (student.containsLecture)
    ...
    void searchStudentsByLecture() {
    		...
    		kwd = scan.next(); // 검색할 과목을 입력
    		
    		Lecture lecture = null; // -> findLecture 함수로 독립시키는 것이 바람직하다.
    		for (Lecture lec : lectureList)
    			if(lec.matchesLecture(kwd))
    				lecture = lec; // -> 여기까지 - match하는 Lecture 객체를 lecture에 할당한다.
    		...
    		lec.print(); // 과목정보를 출력하고
    		
    		for (Student st : studentList) { // 학생 객체에게 과목을 수강하는지 확인한다.
    			if (st.containsLecture(lecture))
    				st.print(); // 해당하는 학생을 출력한다.
    		}		
    }

    // 탑다운 방식이다.
    // 우선 Student Class에 searchStudentsByLecture 메소드를 만들고,
    // 필요한 lec.matchesLecture 및 st.containsLecture를 이후에 생성한다.
    ```
...
- 구현
- 사용자가 검색할 요일을 입력한다.
- 해당 요일에 수업이 있는 학생들을 이름만 출력한다. (`st.name`으로 접근 가능하다. `st.getName()` 메소드를 굳이 만들 필요가 없다. 단, name 필드를 private이 아닌 default로 지정해야 한다.)
- 메소드 호출 흐름
    - 학과 Class : 학생, 너 O요일에 수업이 있니? (matchDays)
    - 학생 Class : 과목, 너 O요일이니? (matchesLecture)

        ```java
        >> 검색할 요일 : 월 <- 입력
        월요일 과목을 수강하는 학생 : 김성관 김재훈 <- 출력
        ```

        ![OOP%20(Java)%20dd6061d9d8954db2839d6acfe1958a90/Untitled%2014.png](OOP%20(Java)%20dd6061d9d8954db2839d6acfe1958a90/Untitled%2014.png)

### 검색-3. 타 학년 과목을 듣는 학생수

- 각 과목에 대해 개설학년과 다른 학년의 수강생 수
    - 과목별로
    - 수강생을 차례로 검사하면서 (전체 학생 loop를 통해 1) 해당 과목을 듣는지, 2) 과목 학년과 일치하는지 확인한다.)
    - 수강생의 학년과 과목의 학년이 다르면 숫자를 +1 증가
    - 과목 정보 출력 후 그 인원수를 출력한다.

        ```java
        DD754 창의기초설계 1 월 123 3명
        DD123 C프로그래밍 1 화 678 2명
        DD834 시스템소프트웨어 2 수 123 1명
        DD982 자료구조설계 2 목 678 4명
        ```

- 알고리즘 (고난이도)
    1. 과목 x에 대해 `for (Lecture x : LectureList) {}`
    2. 인원을 0으로 초기화
    3. 전체 학생 loop를 반복하면서 학생 y에게 과목 x를 듣는지 확인한다.
    4. 과목 x를 듣는 학생인 경우 `Student.containsLecture`
    5. y의 학년과 x의 학년이 다르면, 인원을 증가시킨다. `y.year != x.year` 형태로 접근 가능하다. (y.differYear() 등 메소드를 만들어도 재사용 가능성이 없다고 판단함)
    6. 과목정보 및 인원수를 출력한다.
- 단계별 메뉴 구성
- 대분류 (1) 학생 (2) 과목 (3) 연관검색
- (3)연관검색 메뉴 - (1) 과목별 수강생 (2) 요일별 수강생 (3) 타학년 과목 수강생수 조사
